 ...
 * 
 */
Ext.define('Buzzor.view.logger.LogConfigurationTabPanelController', {
		...
		Ext.Ajax.request({
			...
			success : function(response, opts) {
				...
				if (responseJson.response.success) {
						...
						var parsedFormJSON = JSON.parse(data);
						
						currentObject.submitButton.setText("Update Logger");
		...
		if (form.isValid()) {
			...
			Ext.Ajax.request({
						...
						success : function(response, opts) {
							...
							var responseJson = Ext.JSON.decode(response.responseText);
							if (responseJson.response.success) {
								Ext.Msg.alert("Logger Update",responseJson.response.message);
							} 
							else 
							{
								Ext.Msg.alert("Error...",responseJson.response.message);
