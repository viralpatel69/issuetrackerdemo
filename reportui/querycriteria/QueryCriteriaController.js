Ext.define('Buzzor.view.reportui.querycriteria.QueryCriteriaController', {
	...
	init : function() {
		...
		this.reportViewController = this.getView().up().controller;
	...
	renderComboboxStore : function(com, param) {
		var response = this.reportViewController.syncAjaxPOSTCall(this.reportViewController.restURL
				+ com.serviceUrl, param);
		if (response == null) {
			return;
		}
	...
	loadDataInWidget : function(widget) {
		var response = this.reportViewController.syncAjaxGETCall(this.reportViewController.restURL
				+ widget.serviceUrl, "jsonData={}");
		if (response == null) {
			return;
		}
